...
void HALL_Init(TIM_HandleTypeDef *htim_hall) {
    ...
    uint8_t Hall_B = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4);
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim){
    if(htim == HALL_htim){
        ...
        if(new_sector >= 1 && new_sector <= 6) {
           // İleri Yön Kontrolü (1->2, 2->3 ... 6->1)
           if ((sector == 1 && new_sector == 2) || (sector == 2 && new_sector == 3) ||
           (sector == 3 && new_sector == 4) || (sector == 4 && new_sector == 5) ||
           (sector == 5 && new_sector == 6) || (sector == 6 && new_sector == 1)) {
           direction = 1; 
        } 
        // Geri Yön Kontrolü (1->6, 6->5 ... 2->1)
        else if ((sector == 1 && new_sector == 6) || (sector == 6 && new_sector == 5) ||
             (sector == 5 && new_sector == 4) || (sector == 4 && new_sector == 3) ||
             (sector == 3 && new_sector == 2) || (sector == 2 && new_sector == 1)) {
           direction = -1; 
        }
...
float HALL_GetElectricalAngle(void){
    ...
    if (direction == 1) { // İleri dönüyorsa açıyı ekle
    Total_Angle = (float)HALL_SECTOR_MAP[sector] + Sector_Angle_Inter;
    }
    else {              // Geri dönüyorsa açıyı çıkar
    Total_Angle = (float)HALL_SECTOR_MAP[sector] - Sector_Angle_Inter;
    }

    // 0-360 derece sınırlarına sarma (Wrapping) işlemi
    if(Total_Angle >= 360.0f) Total_Angle -= 360.0f;
    if(Total_Angle < 0.0f)  Total_Angle += 360.0f;
...
float HALL_GetSpeed_RPM(void){
    ...
    if (sector_duration == 0 || (HAL_GetTick() - last_capture_time) > 100) {
    ...
    return rpm * (float)direction; // Yönü de hesaba katıyoruz
}

//  <<<------------------------------------------------------------------------------->>>

float HALL_GetElectricalAngle_Rad(void){
    float angle_deg = HALL_GetElectricalAngle();
    return angle_deg * (PI / 180.0f); // Dereceyi Radyana çevirir
}
...
uint8_t HALL_GetCurrent_Sector(void){
    return sector;
}
...
// Trigger Source: TI1F_ED (Timer Input 1 Edge Detector)